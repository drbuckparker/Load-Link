...
**Platform constraints (unchanged):** the in-app sound listener is iOS-only by existing design (`app/_layout.tsx` returns early on android/web). A custom lock-screen/background push sound additionally needs the sound bundled via the `expo-notifications` app.json plugin (`sounds: [...]`) AND a native rebuild (Expo Launch) — it does NOT work in Expo Go.
