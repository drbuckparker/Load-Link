...
The Replit Git pane's conflict resolver is unreliable for this repo's GitHub sync: marking files resolved doesn't actually stage them, "Complete merge and commit" stays greyed, and it throws "Unknown Error / INVALID_STATE", often aborting the merge. **Reliable path: do the merge in the Shell tab.** The GitHub remote is `subrepl-3d1sr7t6` → github.com/drbuckparker/Load-Link. Pattern: `git pull --no-rebase subrepl-3d1sr7t6 main`, resolve each conflict (`git checkout --theirs <file>` / `git rm <file>`), `git add`, `git commit --no-edit`, then push (UI Push works once the merge is committed). The user's interactive Shell is NOT subject to the agent's destructive-git block, so the user runs these; the agent cannot.
