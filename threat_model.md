...
LoadLink Mobile Companion is a public Expo/React Native client backed by a public Express API deployed on Replit. The API proxies authentication to the main LoadLink website, stores local session mappings in `.data/sessions.json`, reads and writes business data in PostgreSQL, and calls third-party services including Google Maps and Expo Push. Production scope is the deployed mobile client plus the Express API in `server/`; dev artifacts such as `server/routes.ts.bak`, screenshots, and local experimental assets are out of scope unless separately exposed.

## Assets

- **User accounts and session tokens** — local bearer tokens, upstream website JWTs, and the ability to impersonate drivers, contractors, foremen, trucking companies, or admins.
...
- **Highest-risk code areas:** auth/session proxying in `server/routes.ts`; ownership checks across job, assignment, message, vehicle, invoice, and project routes; session persistence in `.data/sessions.json`; weight-ticket and job-run handling.
- **Surface split:** public auth endpoints (`/api/auth/*`, `/api/map-embed`) versus authenticated CRUD endpoints under `/api/*`.
...
Authenticated users can submit updates for jobs, assignments, vehicles, projects, invoices, reviews, availability, and job runs. The API must enforce object ownership and role-specific permissions on every state-changing route. Client-visible role toggles and client-selected IDs are not trusted authorization signals.

### Information Disclosure

The database contains PII, operational messages, location history, invoice parties, job assignments, and weight-ticket images. Read endpoints must only return records for the authenticated principal or an explicitly authorized related party. Message threads, assignment rosters, weight tickets, and invoices are high-sensitivity data sets.
