...
### Core Features
- **Job Management**: Drivers can browse, accept, clock-in/out, and track earnings. Contractors can post, manage assignments (approve/reject drivers with conflict re-check and fleet truck assignment), and view invoices. **Double-booking is enforced server-side** (`server/routes.ts`) for both trucks AND drivers: neither a truck nor a driver can be `approved` on two active jobs whose working days overlap. The truck guard (`findApprovedTruckConflicts`) runs on job accept (auto-approve), assignment approve, and assignment vehicle-change. The driver guard (`findApprovedDriverConflicts`, keyed on `driver_id`, same day-overlap logic) runs on the two paths where a driver becomes approved — job accept (auto-approve) and assignment approve — returning HTTP 409 with a user-facing message (`driverConflictMessage`, with driver-facing vs contractor-facing wording). Both guards return HTTP 409. Only `approved` assignments block; plain `pending` applications stay free and are checked at approval time. Overlap is day-level (a driver/truck booked for a day is booked) — these are day-bookings, not time-of-day slots. Known gap: check-then-write is not transactional, so a true simultaneous double-approval is not prevented (acceptable for the single-contractor workflow). Jobs and vehicles use soft-delete (archive) instead of hard delete — `archived_at` timestamp column on both `jobs` and `trucks` tables. Archived items can be restored.
