...
- **Role switching & entitlement**: `PUT /api/profile/role` is gated by `allowedRolesForUser(originalRole)` (captured at login). The switched role is **never persisted to `users.role`** — the active view lives only in the in-memory session + `sessions.json`. Because allowed roles are derived from `users.role` at every login, persisting a single active view would collapse a compound entitlement (e.g. `driver_trucking_company_contractor`) to one role and trap the account. `GET /api/profile` and `GET /api/auth/me` return the in-memory session user, so in-session switches stick.
