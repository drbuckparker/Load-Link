...
### Core Features
- **Job Management**: Drivers can browse, accept, clock-in/out, and track earnings. Contractors can post, manage assignments (approve/reject drivers with conflict re-check and fleet truck assignment), and view invoices. **Double-booking is enforced server-side** (`server/routes.ts`): a truck can never be `approved` on two active jobs whose working days overlap. The guard (`findApprovedTruckConflicts`) runs on job accept (auto-approve), assignment approve, and assignment vehicle-change, returning HTTP 409 with a user-facing message. Only `approved` assignments block; plain `pending` applications stay free and are checked at approval time. Known gap: check-then-write is not transactional, so a true simultaneous double-approval is not prevented (acceptable for the single-contractor workflow). Jobs and vehicles use soft-delete (archive) instead of hard delete — `archived_at` timestamp column on both `jobs` and `trucks` tables. Archived items can be restored.
