...
**Round 2 (May 2026):** trimmed further from 22 → 16 callers per agent's spot-check verdicts:
- DROPPED: `POST /api/conversations/:jobId/{archive,unarchive,delete}` (no website route exists), `PATCH /api/job-runs/:runId`, `DELETE /api/job-runs/:runId`, `POST /api/job-runs/:runId/weight-tickets` (no website routes — silent 404s)
- PATH FIX: `POST /api/messages/:jobId` → `POST /api/jobs/:jobId/messages` (real route on website)
- KEPT: `POST /api/me/availability` (real cascade), `POST /api/jobs/:id/messages` (WS broadcast)

**Open issue — companion API key mismatch with website:** Direct probes of `https://loadlinklive.com` show our `X-API-Key` value (sent on every push from `server/sync.ts:websiteFetchWithStatus`) is rejected by the website's canonical companion endpoint `POST /api/companion/auth/login` with `401 {"error":"Invalid API key"}`. Consequently every state-changing POST/PUT/DELETE returns `403 {"error":"CSRF token required","code":"CSRF_TOKEN_REQUIRED"}` because the website's CSRF bypass (`isCompanionApiRequest` → `req.headers['x-api-key'] === process.env.COMPANION_API_KEY`) never matches. `GET /api/jobs` returns 200 (read endpoints are public-ish). Companion env has both `WEBSITE_API_KEY` and `COMPANION_API_KEY` (length 64 each) — `websiteFetchWithStatus` prefers `WEBSITE_API_KEY`. Either the website rotated `COMPANION_API_KEY` since the last successful push (~10 hours ago) or the companion's `WEBSITE_API_KEY` was set to a stale value. **No code change resolves this — needs the secret on one side or the other rotated to match.**
