...
**Open issue — website CSRF on POST routes:** During the post-fix drain, all 6 keeper retries (5× `POST /api/jobs/:id/accept`, 1× `POST /api/job-assignments/:aid/cancel`) returned `HTTP 403 {"error":"CSRF token required - please authenticate and fetch token first","code":"CSRF_TOKEN_REQUIRED"}` — yet `POST /api/jobs` (201), `PUT /api/contractor-projects/:id` (200), and `DELETE /api/jobs/:id` (200) succeeded the same day. Companion sends `X-API-Key` + `Authorization: Bearer <jwt>` on every push (no CSRF token). The website appears to apply CSRF middleware to a subset of POST routes; needs to either bypass when `X-API-Key` is present or expose a token-fetch endpoint companion can call. Pending feedback from the website agent.
